...
- [ ] `pnpm add -D jsdom @testing-library/react @testing-library/dom @testing-library/user-event
    @testing-library/jest-dom`.
- [ ] Write the first test in `SyncTable.test.tsx`: given one sync, the row named after its serial
...
- [ ] `vitest.config.ts`: projects `node` (`src/**/*.test.ts`, `data/**/*.test.ts`,
...
- [ ] `test-setup.ts`: `@testing-library/jest-dom/vitest`, `afterEach(cleanup)`, and the
      `matchMedia`, `ResizeObserver` and `scrollIntoView` stand-ins.
- [ ] `test-helpers.tsx`: `renderWithProviders(ui: ReactNode)` renders inside
      `<MantineProvider theme={theme}>`; re-export `screen`, `within` and `userEvent`.
- [ ] The test passes. Break `SyncTable` (swap two columns' values) to see it fail, restore. Commit.

## Task 2: relative dates and the empty state

Files: `SyncTable.test.tsx`, `src/features/device-syncs/ui/SyncTable.tsx`.

- [ ] Add the date tests with `vi.useFakeTimers({ toFake: ['Date'] })` and `vi.setSystemTime`:
...
- [ ] Write the empty-list test: `renderWithProviders(<SyncTable syncs={[]} />)` shows
      "No syncs yet". Watch it fail.
- [ ] `SyncTable`: when `syncs` is empty, one row with a cell spanning the eight columns and the
...
- [ ] `CLAUDE.md`, under the test rules: what gets a component test, what does not, how to query,
      what is banned, and the map exception, as in the spec.
- [ ] `README.md`: the tree shows `SyncTable.test.tsx`; one sentence on component tests.
- [ ] ADR 0003: a short part on component tests, with the two approaches set aside.
- [ ] `pnpm test`, `pnpm exec tsc --noEmit`, `pnpm format`, `pnpm build`, then
      `grep -rli "testing-library\|jsdom" dist/` prints nothing. Commit.
- [ ] One round of review, fixes, push, pull request. Then the deck on `gh-pages`: the Vitest row,
