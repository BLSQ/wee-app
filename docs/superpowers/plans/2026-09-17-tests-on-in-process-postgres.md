...
- [ ] Write `testing.test.ts`, and watch it fail because `./testing` does not exist:
...
- [ ] `migrate.ts`: `migrate(db: Kysely<Database>): Promise<void>`, which no longer creates or
...
- [ ] Rewrite `queries.test.ts` with inserted rows: the limit, newest first (two syncs with explicit
...
- [ ] `migrations.test.ts`: `createTestDb()` instead of `testDb()`.
- [ ] `seed(db)` takes a `Kysely<Database>`. `seeded-database.test.ts` runs it on PGlite and keeps
      its three assertions. Time it: over five seconds, delete the file instead.
- [ ] `prepare.test.ts`: keep the timeout test only.
- [ ] Stop the check: `TEST_DATABASE_URL= pnpm test` passes. Commit.
...
- [ ] Delete `testDb()`. `db-reset.ts` handles `DATABASE_URL` only. Remove `TEST_DATABASE_URL` from
      `.env.example`. Delete `vitest.setup.ts` and its `setupFiles` entry if no test reads `.env`
      any more.
- [ ] `git grep -n "TEST_DATABASE_URL\|wee_app_test\|testDb"` returns only the spec and this plan.
- [ ] `pnpm test`, `pnpm exec tsc --noEmit`, `pnpm format`, `pnpm build`; then
...
- [ ] 0003: queries are tested on an in-process Postgres; name the PGlite and Neon gap.
- [ ] 0006: the seed serves `pnpm dev` and the previews; remove "Determinism is load-bearing".
- [ ] 0008: remove the paragraph about the read-only assumption.
- [ ] 0013: one local database; tests need no Docker; drop the shared test database remarks.
- [ ] `CLAUDE.md`: the two test rules. `README.md`: the `db:reset` comment and the test section.
